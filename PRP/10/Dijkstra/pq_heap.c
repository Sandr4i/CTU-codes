...
{
   pq_heap_s *pq = malloc(sizeof(pq_heap_s));
   if (!pq)
   {
   ...
   pq->size = size;
   pq->len = 0;
   pq->cost = malloc(sizeof(int) * size);
   pq->label = malloc(sizeof(int) * size);
   pq->heapIDX = malloc(sizeof(int) * size);
...
_Bool pq_update(void *_pq, int label, int cost)
{
   _Bool ret = false;
   pq_heap_s *pq = (pq_heap_s*)_pq;
   if ( pq && (pq->len < pq->size) && (label >= 0) && (label < pq->size) && (pq->heapIDX[label] != -1)) //vertex with the label is in the pq 
   {
      int i = pq->heapIDX[label];
      pq->cost[i] = cost; 
      pq->cost[pq->heapIDX[label]] = cost;
      int now = i;
      int parent = GET_PARENT(now);
      while (now >= 1 && pq->cost[parent] > pq -> cost[now])
      {
         pq_swap(pq, parent, now);
         now = parent;
         parent = GET_PARENT(now);
      }
   }
   return ret;
}
