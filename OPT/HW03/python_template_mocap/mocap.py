...
def fitaff(A: np.ndarray, k: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    b0 = A.mean(axis=1)
    A_center = A - b0[:, None]
    U = np.linalg.svd(A_center)[0]
    U = U[:, :k]
    C = np.dot(U.T, A_center)
...
def erraff(A: np.ndarray) -> np.ndarray:
    ...
    m, n = A.shape
    b0 = np.mean(A, axis=1)
    U, S, V = np.linalg.svd(A - b0.reshape(-1, 1), full_matrices=False)
    lambdas = np.sort(np.diag(np.diag(S) **2))[::-1]
    errors = np.zeros(lambdas.shape[0])
    for k in range(m):
       errors[k] = np.sum(lambdas[k+1:])
    return errors
