...
def ar_fit_model(y: np.ndarray, p: int) -> np.ndarray:
    ...
    """
    b = y[::-1]
    ones = np.ones(len(y) - p)
    sub_matrix = [b[i:i+p] for i in range(len(y) - p, 0, -1)]
    stacked = np.vstack(sub_matrix).T
    M = np.vstack([ones, stacked]).T
    a = np.linalg.lstsq(M, y[p:].T, rcond=None)[0]
    return a
...
def ar_predict(a: np.ndarray, y0: np.ndarray, N:int) -> np.ndarray:
    ...
    y_pred[:p] = y0
    for i in range(p, N):
        y_pred[i] = np.dot(a[1:], y_pred[i-p:i][::-1]) + a[0]
    return y_pred
