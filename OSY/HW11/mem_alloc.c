...
void *brk(void *address){
  ...
  asm volatile (
      ...
      "mov $1f, %%edx"
      "sysenter"
      "1:" : "+a"(w0) : "S"((unsigned*)address) : "ecx", "edx", "memory");
    return (void*)(unsigned long)w0;
}
...
void *my_malloc(unsigned long size){
  ...
  // run out of space, allocate more
  ...
  // found an empty block, split the block, use one, let the other one empty
  if(*((mword*)current) != 0){
    mword blockSize = *((mword*)current);
...
int my_free(void *address){
  ...
  if(*(x - HEADER_SIZE + FLAGS_OFFSET) == 0)
    return -1; // trying to free a free block
